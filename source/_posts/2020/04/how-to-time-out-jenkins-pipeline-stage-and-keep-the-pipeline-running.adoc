...
We can be very specific - in this case we catch only the `FlowInterruptedException`, so any other exception will stop the pipeline anyway.
And when we catch the timeout, we emit the error pass:[<em class="conum" data-value="3"></em>] so that the `catchError` step can do its job.
