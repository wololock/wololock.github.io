...
In this blog post, I will explain why I decided to switch, and how to set up CircleCI to deploy Hexo blog to the GitHub Pages without much hustle.
...
However, I have found two major reasons to start considering alternative solutions.
...
I made some experiments, and it turned out that CircleCI could solve both problems.
...
NOTE: Be careful when you generate a new key.
It will ask you where to save the key file - do not override the default `~/.ssh/id_rsa` one.
This is your personal key, and you don't want to lose that one.
Also, use an empty passphrase, so there is no prompt for the password needed.

[source,bash]
----
$ ssh-keygen -t rsa -m PEM -C "bot@circleci.com"
...
SHA256:3K/RQpfR0sdJ4hv78JL/VSB10up0tuzTZnpvRodVtV0 bot@circleci.com
...
Here is my question to you: what continuous integration server would you use to deploy a generated static site, and why?
