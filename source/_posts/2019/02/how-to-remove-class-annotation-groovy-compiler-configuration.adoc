...
NOTE: Kudos to https://twitter.com/sbglasius[Søren Glasius] who bring the question about removing annotations at the compile time to the *Groovy Community Slack*!footnote:[https://groovycommunity.com/]
...
Instead, let's use compiler configuration script to do it for us (without making a single change to the source code).
...
If we take a compiled class file and we decompile it, we get bytecode represented as the following Java code equivalent.
...
A Groovy compiler configuration script gives you a lot of options to customize compiler behavior.
